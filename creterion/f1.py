...
from loss.deepblinkloss import rmse
...
def _f1_at_cutoff(
    ...
    if outuse:
        return f1_value,precision,recall,true_pred_r, true_pred_c
...
def compute_metrics_once(
    ...
    rmse = np.sqrt(np.sum(np.square(np.array(c_offset))) / float(len(c_offset)))
    

    return f1_,precis_,recall_,rmse
